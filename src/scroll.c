...
void scroll_update_current(struct raw_coords_x* pCoords)
{
    ...
    if (id != 0x0)
    {
        while (id != 0x0)
        {
            if (pScroll == (struct scroll*)&current_power_bomb)
                return;
            ...
            {
                if (bg_pointers_and_dimensions.clipdata_decompressed[pData[0x5] * bg_pointers_and_dimensions.clipdata_width + pData[0x4]] == 0x0)
                {
                    if (pData[0x6] != 0xFF)
            ...
            if (pData[bounds[0x0]] <= x_pos && x_pos <= pData[bounds[0x1]] && pData[bounds[0x2]] <= y_pos && y_pos <= pData[bounds[0x3]] && pScroll->within == 0x0)
            {
                ...
                // X end
                clip_limit = bg_pointers_and_dimensions.clipdata_width << 0x6 - 0x80;
                ...
                // Y end
                clip_limit = bg_pointers_and_dimensions.clipdata_height << 0x6 - 0x80;
