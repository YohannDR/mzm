...
u32 TitleScreenFadingIn(void)
{
    ...
    switch (TITLE_SCREEN_DATA.unk_10)
    {
        ...
        case 1:
            ...
            {
                src = sEwramPointer + 0x8000;
                dst = sEwramPointer + 0x8400;
                ApplySpecialBackgroundFadingColor(0, TITLE_SCREEN_DATA.colorToApply, &src, &dst, 0xFFFF);

                src = sEwramPointer + 0x8200;
                dst = sEwramPointer + 0x8600;
                ApplySpecialBackgroundFadingColor(0, TITLE_SCREEN_DATA.colorToApply, &src, &dst, 0xFFFF);
            ...
            DMATransfer(3, sEwramPointer + 0x8000, sEwramPointer + 0x8400, 0x400, 0x10);
            TITLE_SCREEN_DATA.unk_12 = TRUE;
            TITLE_SCREEN_DATA.unk_10++;
            break;
...
void unk_76710(u8 param_1)
{
    if (!param_1)
    {
        DMATransfer(3, PALRAM_BASE, sEwramPointer + 0x8000, 0x400, 0x10);
        BitFill(3, 0, PALRAM_BASE, 0x400, 0x10);
        DMATransfer(3, PALRAM_BASE, sEwramPointer + 0x8400, 0x400, 0x10);
    }
    else
        DMATransfer(3, PALRAM_BASE, sEwramPointer + 0x8000, 0x400, 0x10);
...
void unk_767a4(void)
{
    if (TITLE_SCREEN_DATA.unk_12)
    {
        DMATransfer(3, sEwramPointer + 0x8400, PALRAM_BASE, 0x400, 0x10);
        TITLE_SCREEN_DATA.unk_12 = FALSE;
    }
}
...
void unk_76978(u8 param_1)
{
    if (param_1 & 1)
    {
        DMATransfer(3, sEwramPointer + 0x4000, VRAM_BASE + 0x4000, 0x800, 0x10);
        DMATransfer(3, sEwramPointer + 0x4800, VRAM_BASE + 0x4800, 0x800, 0x10);
    }

    if (param_1 & 2)
    {
        DMATransfer(3, sEwramPointer + 0x5000, VRAM_BASE + 0x5000, 0x800, 0x10);
        DMATransfer(3, sEwramPointer + 0x5800, VRAM_BASE + 0x5800, 0x800, 0x10);
    }

    if (param_1 & 4)
    {
        DMATransfer(3, sEwramPointer + 0x6000, VRAM_BASE + 0x6000, 0x800, 0x10);
        DMATransfer(3, sEwramPointer + 0x6800, VRAM_BASE + 0x6800, 0x800, 0x10);
    }

    if (param_1 & 8)
    {
        DMATransfer(3, sEwramPointer + 0x7000, VRAM_BASE + 0x7000, 0x400, 0x10);
...
void TitleScreenProcessOAM(void)
{
    ...
    if (TITLE_SCREEN_DATA.type & TITLE_SCREEN_TYPE_BOTTOM_SPARKLE_ACTIVE)
    {
        ...
        {
            UpdateMenuOamDataID(&TITLE_SCREEN_DATA.oam[5], 5);
            UpdateMenuOamDataID(&TITLE_SCREEN_DATA.oam[6], 5);
        }
    }
    ...
    switch (gGameModeSub1)
    {
        ...
        case 2:
            ...
            if (gGameModeSub2 != 0)
            {
                ...
                {
                    SoundPlay(0x213);
                    TITLE_SCREEN_DATA.unk_28[2] = sTitleScreen_760aa0[3];
                }
                gGameModeSub1 = 3;

                if (TITLE_SCREEN_DATA.unk_28[1].unk_4 != 0)
                    TITLE_SCREEN_DATA.unk_28[1].unk_4 = 1;
        ...
        case 3:
            ...
            if (TITLE_SCREEN_DATA.unk_28[2].unk_0 == 0 && TITLE_SCREEN_DATA.timer > 40)
            {
                unk_76710(TRUE);
                gGameModeSub1++;
                TITLE_SCREEN_DATA.timer = 0;
...
void TitleScreenInit(void)
{
    ...
    gMenuOamXOffset = gMenuOamYOffset = 0;
    ...
    // Undefined
    load_special_graphics_data(&sTitleScreenPageData[0]);
    load_special_graphics_data(&sTitleScreenPageData[1]);
    ...
    CallLZ77UncompWRAM(sTitleScreenSpaceAndGroundBackgroundGFX, sEwramPointer + 0x4000);

    DMATransfer(3, VRAM_BASE + 0x4000, sEwramPointer, 0x4000, 0x10);

    CallLZ77UncompVRAM(sTitleScreenSparklesGFX, VRAM_BASE + 0x10000);

    // Undefined
    set_bgcnt_special_graphics_data(&sTitleScreenPageData[0]);
    set_bgcnt_special_graphics_data(&sTitleScreenPageData[1]);
    ...
    TITLE_SCREEN_DATA.unk_A = 5;

    TITLE_SCREEN_DATA.unk_28[0] = sTitleScreen_760aa0[0];
    TITLE_SCREEN_DATA.unk_28[1] = sTitleScreen_760aa0[1];
    TITLE_SCREEN_DATA.unk_28[2] = sTitleScreen_760aa0[2];
...
void unk_777d8(u8 param_1)
{
    ...
    bgOffset = sTitleScreenPageData[0].graphicsPage * 2048;
