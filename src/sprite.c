            ...
            do
            {
                if (gSpriteData[count].status & SPRITE_STATUS_EXISTS)
                {
                    ...
                    if (pCurrent->properties & SP_SECONDARY_SPRITE) // Call AI
                        secondary_sprite_ai_pointers[pCurrent->spriteID]();
                    else
                        primary_sprite_ai_pointers[pCurrent->spriteID]();
...
void SpriteInitPrimary(u8 spritesetSlot, u16 yPosition, u16 xPosition, u8 roomSlot)
{
    ...
    while (pSprite < gSpriteData + 24)
    {
        if ((pSprite->status & SPRITE_STATUS_EXISTS) == 0x0)
        {
            ...
            {
                slot = ((((spritesetSlot - 0x1) << 0x18) & 0xF000000) >> 0x18);
                pSprite->spritesetGFXSlot = gSpritesetGFXSlots[slot];
                spriteID = gSpritesetSpritesID[slot];
            }
            else
            {
 ...
 */
u8 SpriteSpawnSecondary(u8 spriteID, u8 roomSlot, u8 gfx_slot, u8 ramSlot, u16 yPosition, u16 xPosition, u16 status_to_add)
{
    u8 new_ramSlot;
    struct SpriteData* pSprite;
    u16 status;
    ...
    while (pSprite < gSpriteData + 24)
    {
        ...
        if (status == 0x0)
        {
            ...
            pSprite->spritesetGFXSlot = gfx_slot;
            pSprite->spriteID = spriteID;
 ...
 */
u8 SpriteSpawnPrimary(u8 spriteID, u8 roomSlot, u8 gfx_slot, u16 yPosition, u16 xPosition, u16 status_to_add)
{
    u8 new_ramSlot;
    struct SpriteData* pSprite;
    u16 status;
 ...
 * @return The assigned RAM slot of the spawned sprite, 0xFF is the sprite couldn't spawn
 */
u8 SpriteSpawnDropFollowers(u8 spriteID, u8 roomSlot, u8 gfx_slot, u8 ramSlot, u16 yPosition, u16 xPosition, u16 status_to_add)
{
    u8 new_ramSlot;
    struct SpriteData* pSprite;
    u16 status;
