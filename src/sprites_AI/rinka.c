...
void RinkaMove(void)
{
    ...
    if (gCurrentSprite.status & SPRITE_STATUS_FACING_RIGHT)
    {
        if (gCurrentSprite.status & SPRITE_STATUS_UNKNOWN2)
        {
            speed = (u16)(u16)Sqrt(distanceXRight * distanceXRight + distanceYUp * distanceYUp);
            if (speed != 0x0)
            {
                gCurrentSprite.yPosition = spawnY + ((velocity * ((i32)(distanceYUp << 0xA) / speed) >> 0xA));
                gCurrentSprite.xPosition = spawnX + ((velocity * ((i32)(distanceXRight << 0xA) / speed) >> 0xA));
            }
        }
        else
        {
            speed = (u16)Sqrt(distanceXRight * distanceXRight + distanceYDown * distanceYDown);
            if (speed != 0x0)
            {
                gCurrentSprite.yPosition = spawnY - ((velocity * ((i32)(distanceYDown << 0xA) / speed) >> 0xA));
                gCurrentSprite.xPosition = spawnX + ((velocity * ((i32)(distanceXRight << 0xA) / speed) >> 0xA));
            }
        }
    }
    else
    {
        if (gCurrentSprite.status & SPRITE_STATUS_UNKNOWN2)
        {
            speed = (u16)Sqrt(distanceXLeft * distanceXLeft + distanceYUp * distanceYUp);
            if (speed != 0x0)
            {
                gCurrentSprite.yPosition = spawnY + ((velocity * ((i32)(distanceYUp << 0xA) / speed) >> 0xA));
                gCurrentSprite.xPosition = spawnX - ((velocity * ((i32)(distanceXLeft << 0xA) / speed) >> 0xA));
            }
        }
        else
        {
            speed = (u16)Sqrt(distanceXLeft * distanceXLeft + distanceYDown * distanceYDown);
            if (speed != 0x0)
            {
                gCurrentSprite.yPosition = spawnY - ((velocity * ((i32)(distanceYDown << 0xA) / speed) >> 0xA));
                gCurrentSprite.xPosition = spawnX - ((velocity * ((i32)(distanceXLeft << 0xA) / speed) >> 0xA));
